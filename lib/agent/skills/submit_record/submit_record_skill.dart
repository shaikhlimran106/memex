...
4. This tool currently supports text records only. If the user needs to attach photos, audio, or files, ask them to use the existing attachment entry until the Super Agent attachment flow is available.
''';
  }
  ...
  static List<Tool> _buildTools() {
    return [
      Tool(
        ...
        executable: (String content, String? reason) async {
          ...
          _logger.info(
            'Submitting SuperAgent record for user=$userId, reason=${reason ?? '-'}',
          );
